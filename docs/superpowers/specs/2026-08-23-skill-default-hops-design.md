...
**Status:** Approved design (2026-08-23). JT directed this locked B cut. Implementation waits on this spec.
...
| 4 | User/project overlay transitions win per `(from, on)` only. They do **not** replace-by-`from`. Other skill-default hops for that `from` stay. Overlay may set `to: null` or `to: wait` to unhook **one** default hop. | This is why hops can live on the skill: an overlay can change one edge without deleting the rest. |
...
| 7 | Delete `workflows/default.yaml` entirely. Resolver bundled base is empty (`version: 1`, no skills stubs required). Graph = cataloged `next` edges + bundled capability overlays + user/project overlays. | One baseline: the skills. No second YAML graph and no fallback copy of the deleted file. |
...
“Known logical id” is the same set overlay transitions already use: cataloged ids, overlay/registry ids, and bundled skill directory names from `discover_known_skills`. Empty bundled YAML does **not** require identity stubs for those directory names.
...
Cross-cutting skills (TDD, systematic-debugging, verification-before-completion, requesting-code-review, receiving-code-review, dispatching-parallel-agents, writing-skills, using-superpowers, using-git-worktrees, visual-surface) stay description-triggered / remap-only. They do not require `next` in this cut.
...
2. **Empty bundled base** — in memory `{version: 1}`. Do not read `workflows/default.yaml`. Do not synthesize identity stubs. Do not keep a fallback copy of the deleted file.
...
A custom brainstorming remap therefore carries *that* file’s hops under `from: brainstorming`, then overlay transitions may still unhook or replace one `(from, on)`.
...
Project `.supersuit/workflow.yaml` that stops the architectural chain and keeps the rest of brainstorming’s skill-default hops:
...
`approved-bounded` and `approved-spike` stay `null` from brainstorming’s `next`. The overlay does **not** re-list them.

`to: null` on one `(from, on)` is also an unhook: continue-session instead of the skill-default target.
...
Resolved JSON shape after this cut:
...
- Overlay `to: null` or `to: wait` unhooks that one hop.
...
- Bundled YAML base is `{version: 1}`. No skills stubs required in that base.
...
- Delete `workflows/default.yaml`. Resolver starts from `{version: 1}` in memory.
...
- Tests: extra `next` key warns+skips; missing key is `wait`; unknown `to` resolve-errors; per-`(from, on)` overlay unhook leaves sibling hops; gated `native-worktree` still wins; no `default.yaml` read; resolved JSON has no `entries`; foreign cataloged `next` uses the same rules; `--bundled-only` is empty YAML + catalog hops + bundled overlays.
